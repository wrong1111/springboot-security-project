...
return win;
},prompt:function(_270,msg,fn,defualtText){
    if(defualtText==null||defualtText==undefined) defualtText="";
var _271="<div class=\"messager-icon messager-question\"></div>"+"<div>"+msg+"</div>"+"<br/>"+"<div style=\"clear:both;\"/>"+"<div><input class=\"messager-input\" type=\"text\"/ value="+defualtText+"></div>";
var _272={};
...
return win;
},promptRevoke:function(_270,msg,fn,arryText){
    if(arryText==null||arryText==undefined) arryText=[""];
    var option="";
    for(var i=0;i<arryText.length;i++){
        if(i==0){
            option+="<option selected value='"+arryText[i]+"'>"+arryText[i]+"</option>";
        }else{
            option+="<option  value='"+arryText[i]+"'>"+arryText[i]+"</option>";
        }
    }
    var _271="<div class=\"messager-question\"></div>"+"<div> <div style=\"width: 80px;float: left; text-align: right;\"><b>是否发送短信:</b></div><div style=\"float: left; text-align: left; margin-left: 10px;\"><input type=\"radio\" name=\"shortinfo\"  value=\"1\" checked>是&nbsp;&nbsp;<input type=\"radio\" name=\"shortinfo\"  value=\"0\">否</div></div></div>"+"<div style=\"clear:both;\"/>"+"<div><div style=\"width: 80px;float: left; text-align: right;\"><b>原因:</b></div><div style=\"float: left;text-align: left; margin-left: 10px;\"><select class=\"messager-input\" >"+option+"</select></div></div><div style=\"clear:both;\"/>";
    var _272={};
    _272[$.messager.defaults.ok]=function(){
        win.window("close");
        if(fn){
            fn($(".messager-input",win).val(),$('input[name="shortinfo"]:checked',win).val());
            return false;
        }
    };
    _272[$.messager.defaults.cancel]=function(){
    ...
    var win=_264(_270,_271,_272);
    win.children("textarea.messager-input").focus();
    return win;
...
$.fn.datagrid=function(_5e6,_5e7,_func){
if(typeof _5e6=="string"){
 if(_func!=undefined)
   return   $.fn.datagrid.methods[_5e6](this,_5e7,_func);
    else
...
},updateRowFunc:function(jq,_64e,_objFunc){
    return jq.each(function(){
        var opts=$.data(this,"datagrid").options;
        opts.view.updateRow.call(opts.view,this,_64e.index,_64e.row);
        _objFunc.call(this);
...
},showColumns:function(jq,_655,_656){
    return jq.each(function(){

        $(this).datagrid("getPanel").find("td[field=\""+_655+"\"]").show();
        $(this).datagrid("getColumnOption",_655).hidden=false;
//       $(this).datagrid("fitColumns");
        if(_656!=null&&_656!=undefined){
            this.call(_656);
        }
    });
},hideColumn:function(jq,_657){
return jq.each(function(){
var _658=$(this).datagrid("getPanel");
_658.find("td[field=\""+_657+"\"]").hide();
$(this).datagrid("getColumnOption",_657).hidden=true;
$(this).datagrid("fitColumns");
});
},
    hideColumns:function(jq,_659,funObj){
        return jq.each(function(){
            var _660=$(this).datagrid("getPanel");
            _660.find("td[field=\""+_659+"\"]").hide();
            $(this).datagrid("getColumnOption",_659).hidden=true;
    //        $(this).datagrid("fitColumns");
            if(funObj!=null&&funObj!=undefined){
                this.call(funObj);
            }
        });
    }
...
if($.trim(s)==""){
    ...
    return new Date(year,month,date1,0,0,0);
}
var dt=s.split(" ");
